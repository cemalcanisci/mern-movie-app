...
class Order extends Component {
  ...
  onSortEnd = ({ oldIndex, newIndex }) => {
    ...
    const promise = this.props.update(newArr);
  };
  ...
  render() {
    ...
    const sortable = this.state.movies.length ? (
      <Card>
        {' '}
        <SortableList helperClass="active-sort" key={this.state.movies._id} items={this.state.movies} onSortEnd={this.onSortEnd} />
      </Card>
    ) : <div>Yoh</div>;
    return (
      <Card>
